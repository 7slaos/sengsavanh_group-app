...
import 'package:pathana_school_app/models/setting_payment_package_model.dart';
...
class GenerateQrcodePaymentPackage extends StatefulWidget {
  final SettingPaymentPackageModel data;
  final String id;
  final String type;
  final String amount;
  final String qty;
  ...
  final String phone;
  const GenerateQrcodePaymentPackage(
      {super.key,
      required this.id,
      required this.type,
      required this.data,
      ...
      this.note});
  @override
  State<GenerateQrcodePaymentPackage> createState() =>
      _GenerateQrcodePaymentPackageState();
}

class _GenerateQrcodePaymentPackageState
    extends State<GenerateQrcodePaymentPackage> {
  ...
  Future<void> _captureAndSavePng() async {
    try {
      ...
      final result = await ImageGallerySaverPlus.saveImage(pngBytes,
          quality: 100, name: 'cit_schools_lao_qrcode_payment_package');
      if (result['isSuccess']) {
        // ignore: use_build_context_synchronously
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 5), (Timer timer) {
      if (widget.bankId == '1') {
        if (adminPaymentPackageState.laoQrModel != null) {
          adminPaymentPackageState.checkPayment(
              id: widget.id,
              total: widget.amount,
              type: widget.type,
              qty: widget.qty,
              studentCount: widget.data.studentCount.toString(),
              price: widget.data.pricePerStudent.toString(),
              note: widget.note,
              transactionId:
                  adminPaymentPackageState.laoQrModel!.transactionId!);
        }
      }
    });
    super.initState();
  ...
  getData() {
    if (widget.bankId == '1') {
      adminPaymentPackageState.generateLaoQrcode(
      ...
      adminPaymentPackageState.fetchAuthToken(
      ...
      body: Container(
        ...
        child: SingleChildScrollView(
          child: Padding(
            ...
            child: GetBuilder<AdminPaymentPackageState>(builder: (getPay) {
