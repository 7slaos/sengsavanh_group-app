...
class StudentCardPage extends StatelessWidget {
  ...
  static const String? avatarUrl =
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAXaG33Bhqe6mWbT_7PqdO2pQ1gfScozTqJA&s';
  String get qrValue => '16284756'; // use name as QR value
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFF0F4AA6),
      body: GetBuilder<ProfileStudentState>(
          builder: (getPro) {
            return Stack(
              children: [
                const _Background(),
                SafeArea(
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        const _TopBar(title: 'STUDENT ID CARD'),
                          ...
                          child: Center(
                            child: SingleChildScrollView(
                              child: Column(
                                children: [
                                  _Avatar(
                                      name: '${getPro.dataModels?.firstname ?? ""} ${getPro.dataModels?.lastname ?? ""}',
                                      url: ''),
                                  CustomText(
                                  ...
                                  CustomText(
                                    text: '${"academic_year".tr}: ${currentAcademicYear(startMonth: 9)}',
                                    textAlign: TextAlign.center,
                                    color: Colors.white,
                                    fontSize: 24,
                                    fontWeight: FontWeight.w800,
                                  ...
                                  CustomText(
                                    text: '${getPro.dataModels?.firstname ?? ""} ${getPro.dataModels?.lastname ?? ""}',
                                    textAlign: TextAlign.center,
                                    color: Colors.white,
                                    fontSize: 24,
                                    fontWeight: FontWeight.w800,
                                  ...
                                  CustomText(
                                    text: getPro.dataModels?.myClass ?? '',
                                    color: Colors.white70,
                                    fontSize: 16,
                                    fontWeight: FontWeight.w600,
                                  ),
                                  const SizedBox(height: 8),
                                  QrCard(value: getPro.dataModels?.code ?? qrValue),
                                  const SizedBox(height: 18),
...
class _Background extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: CustomPaint(
          painter: _CirclesPainter(), child: const SizedBox.expand()),
    );
  }
}
...
class QrCard extends StatelessWidget {
  const QrCard({super.key, required this.value});
  final String value;

  @override
  Widget build(BuildContext context) {
    final data = value.isEmpty ? 'fallback' : value;
    Size size = MediaQuery.of(context).size;
    ...
    return Row(
      ...
      children: [
        ...
        Container(
          ...
          padding: const EdgeInsets.all(8),
          child: BarcodeWidget(
            // Code128 supports letters, numbers, hyphens, etc.
            barcode: Barcode.code128(),
            data: data,
            width: size.width * 0.3,
            height: size.width * 0.3,
            drawText: true, // show human-readable text under bars
          ),
        ),
