...
class _ScanResultPageState extends State<ScanResultPage> {
  static Color kRed = AppColor().mainColor;
  final AppColor appColor = AppColor();
  final ProfileTeacherState profileTeacherState =
      Get.put(ProfileTeacherState());
      ...
      return Scaffold(
                ...
                onPressed: () {
                  Get.to(
                      () => CameraScanPage(
                          type: widget.type,
                          student_records_id:
                              getPro.dataModels?.studentRecordsId, id: widget.id),
        ...
        body: GetBuilder<CheckInOutStudentState>(builder: (checkIn) {
          return GetBuilder<ProfileTeacherState>(
            builder: (tController) {
              final teacher = tController.teacherModels;
              ...
              return GetBuilder<ProfileStudentState>(builder: (sController) {
                ...
                return Stack(
                  children: [
                        ...
                        if (loading)
                          Center(
                            child: LoadingAnimationWidget.staggeredDotsWave(
                                color: appColor.mainColor, size: 70),
                          ),
                        if (!loading) ...[
                          _ResultCard(
                            scale: scale,
                            day: day,
                            month: laoMonthName(int.parse(month.toString())),
                            year: year,
                            studentName: widget.type == 't'
                                ? teacherFullName.isEmpty
                                    ? '—'
                                    : teacherFullName
                                : studentFullName.isEmpty
                                    ? '—'
                                    : studentFullName,
                            teacherId: teacher?.code ?? null,
                            studentId: student?.code ?? null,
                          ...
                          if (checkIn.dataList.isNotEmpty) ...[
                            for (var i = 0; i < checkIn.dataList.length; i++)
                              _ResultCard(
                                ...
                                day: day,
                                month:
                                    laoMonthName(int.parse(month.toString())),
                                year: year,
                                studentName:
                    ...
                    if (!loading &&
                        checkInOutStudentState.checkHosLiDay == false)
                      Positioned(
                        right: 20 * scale,
  ...
  /// Optional: great-circle distance in KM using Haversine
  /// Use with parsed doubles:
  ///   final km = haversineKm(17.97, 102.63, 18.28, 103.55);
...
class _LocationPanel extends StatelessWidget {
  ...
  Widget _kv(String k, String v) => Row(
        children: [
          Text(k,
              style: TextStyle(
                  fontSize: 13 * scale, color: Colors.black.withOpacity(.7))),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      (teacherId != null && teacherId.toString() != '')
                          ? 'ລະຫັດບຸກຄະລາກອນ: $teacherId'
                          : 'ລະຫັດນັກຮຽນ: ${studentId ?? ""}',
                      style: TextStyle(
                          fontSize: 13 * scale, color: Colors.black87),
                    ),
                  ],
                ),
              ),
...
class _SmallRoundBtn extends StatelessWidget {
  const _SmallRoundBtn({required this.color, required this.icon, this.onTap});
  final Color color;
  final IconData icon;
